...
def information(request):
    try:
        print(request.POST['email_jamer'])
        participante = Participantes.objects.filter(email = request.POST['email_jamer'])
        print(participante[0].apellidos)
        return render(request, 'informacion.html', {
            'id':participante[0].id,
            'nombre':participante[0].nombres + ' ' + participante[0].apellidos,
            'correo':request.POST['email_jamer'],
            'rol':participante[0].rol
            })
    except:
        return render(request, 'informacion.html',{})